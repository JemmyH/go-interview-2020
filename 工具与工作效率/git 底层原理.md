...
标签就是一次提交的快照。通常它保存的是需要追溯的特定版本数据的一个 `commit` 对象的数字签名。它非常类似于一个 `commit` 对象——它包含一个标签创建者信息、一个日期、一段注释信息，以及一个指针。主要区别在于，标签对象通常指向一个 `commit` 对象，而不是一个 `tree` 对象。它像是一个永不移动的分支引用——**永远指向同一个提交对象，只不过给这个提交对象加上一个更友好的名字罢了**。
...
标签引用的作用就是给 `git` 对象打标签，便于记忆。先看我们的 `log`：

```bash
$ git log --pretty=oneline
2f42ff5d10653abb49c97a316ec50bef69897048 (HEAD -> master) add dev file
21cc60f134b4d1cf898905790ad12e51ec8c6a17 update 1.txt version 2
4741fa26a05b943f1d7068ae8c6909fdc6ca908a update myfile t4
629be158cf9db84a8857559406e30e0c924e86b5 update myfile t3
6d8b3c6beb1011636620357b045d2512cf240e37 update myfile
...
我们想给 `update 1.txt version 2` 这个提交打一个标签，比如 `v1.0.1`，可以这么做：

```bash
$ git update-ref refs/tags/v1.0.1 21cc60f134b4d1cf898905790ad12e51ec8c6a17

# 查看 git log
$ git log --pretty=oneline
2f42ff5d10653abb49c97a316ec50bef69897048 (HEAD -> master) add dev file
21cc60f134b4d1cf898905790ad12e51ec8c6a17 (tag: v1.0.1) update 1.txt version 2
4741fa26a05b943f1d7068ae8c6909fdc6ca908a update myfile t4
629be158cf9db84a8857559406e30e0c924e86b5 update myfile t3
6d8b3c6beb1011636620357b045d2512cf240e37 update myfile
bb422cd714c2d90ff6db2be782539d9f4d8d6b78 new myfile
...
21cc60f134b4d1cf898905790ad12e51ec8c6a17
```

有时候我们想给这个标签加一些说明，比如某个版本的 `release`，可以使用下面的方式：

```bash
$ git tag -a v2.0.0 -m "1. 新增功能 xxx 2. 修复 bug"
```

这种标签引用叫做 “附注引用”，特点是可以为标签增加说明信息。与常规标签不同的是(常规标签直接指向提交对象)，附注标签不直接指向一个 `commit对象`，而是在 `.git/objects` 下新建了一个 `blob` 对象用来记录附注信息，然后附注标签指向这个对象。

```bash
$ cat .git/refs/tags/v2.0.0
be1a58cfca8b041a6747580188a1f66de6422fcf

$ git cat-file -p be1a58cfca8b041a6747580188a1f66de6422fcf
object 2f42ff5d10653abb49c97a316ec50bef69897048
type commit
tag v2.0.0
tagger Jemmy <hujm20151021@gmail.com> 1604668098 +0800

1. 新增功能 xxx 2. 修复 bug

$ git cat-file -p 2f42ff5d10653abb49c97a316ec50bef69897048
tree ce9237b59ef5f38b9e87bb0b629c83351d026263
parent 21cc60f134b4d1cf898905790ad12e51ec8c6a17
author Jemmy <hujm20151021@gmail.com> 1604375975 +0800
committer Jemmy <hujm20151021@gmail.com> 1604375975 +0800

add dev file
# 正是最后一次提交
```

需要注意的是，**标签引用** 并不只能给 `commit` 打标签，实际上它可以指向 4 中对象中的任何一种，即可以给任何一个 `git对象` 打标签。

#### 3.3 远程引用

`.git/refs/remotes` 用来记录多个远程仓库各分支的最后一次提交。比如，我们添加一个叫做 `origin` 的远程版本库，然后把 `master` 分支推送上去:

```bash
$ git remote add origin https://github.com/JemmyH/learn_git.git
$ git push -u origin master
Enumerating objects: 40, done.
Counting objects: 100% (40/40), done.
Delta compression using up to 8 threads
Compressing objects: 100% (21/21), done.
Writing objects: 100% (40/40), 3.04 KiB | 444.00 KiB/s, done.
Total 40 (delta 1), reused 37 (delta 1)
remote: Resolving deltas: 100% (1/1), done.
To https://github.com/JemmyH/learn_git.git
...
2f42ff5d10653abb49c97a316ec50bef69897048
```

远程引用和分支（位于 `refs/heads` 目录下的引用）之间最主要的区别在于，远程引用是只读的。 虽然可以 `git checkout` 到某个远程引用，但是 `git` 并不会将 `HEAD引用` 指向该远程引用。因此，你永远不能通过 `commit` 命令来更新远程引用。 `git` 将这些远程引用作为记录远程服务器上各分支最后已知位置状态的书签来管理。
...
有一个项目，最开始是 `master` 分支，最新的一个 tag 是 `v1.0.6`。

有一天，有了新的需求，经过了需求评审和技术评审，确定新的版本号为 `v2.x.x`，于是小胡开始了他长达两周的开发。他在 `master` 的基础上 `checkout` 出一个新的分支 `feat/v2`，因为是一个大版本的开发，小胡删了很多不用的代码，也增加了很多新的功能，改动特别大。

但是在第一周的时候，线上运行的 `v1.0.6` 版本有一些问题需要紧急修复，新功能又没开发完，所以小胡在 `master` 基础上切了一个新的 `bug-fix` 分支去修复，通过 `code review` 之后合并到 `master` ，并打 `tag` 为 `v1.0.7` 。

两周之后，新的功能开发完成了，小胡面临的最大问题是：如何将 `feat/v2` 的代码合并到 `master`。他的步骤是：

1.  基于 `feat/v2` 切除一个新分支 `merge-featv2`；
2.  在 `merge-featv2` 分支上，执行 `git rebase master`，在这个过程中不断的 `解决冲突 -> git add 冲突文件 -> git rebase --continue`；
3.  在 `merge-featv2` 分支上，执行 `git merge feat/v2`，这个过程中也会有冲突，解决冲突，之后 `git commit` 提交冲突信息；
4.  再切回 `feat/v2`，执行 `git rebase merge-featv2 ` (或者 `git merge merge-featv2`，区别不大 )；
5.  最后将 `feat/v2` 合并到 `master` ，没有冲突。

在这个过程中有两点需要注意：1. `rebase` 和 `merge` 的区别 2. 算是一个小技巧：使用新的分支专门进行冲突解决，这样即使合并失败，也可以直接丢弃掉从头再来，如果在 `master` 或者 开发分支 上做合并与冲突解决工作，万一失败苦不堪言。
...
- [Git 内部原理之 Git 引用](https://blog.csdn.net/xiamiflying/article/details/84147046)